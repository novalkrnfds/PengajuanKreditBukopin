<?php
    ...
    $selectkredit = mysql_query("SELECT a.id_kredit, max(a.datetime) as datetime, MAX(a.status) AS status, b.*, c.*
    FROM tb_status_validasi a INNER JOIN tb_kredit b ON a.id_kredit = b.id_kredit INNER JOIN tb_nasabah c ON b.id_nasabah = c.id_nasabah WHERE c.id_nasabah = '$idnasabah'");
    $disable = mysql_fetch_array($selectkredit);
    ...
    if(isset($_POST['savepekerjaan'])){
        if($_POST['id_pekerjaan'] == ""){
            ...
            $updatepekerjaan = mysql_query("update tb_pekerjaan set nama_perusahaan = '$_POST[nama_perusahaan]', alamat_perusahaan = '$_POST[alamat_perusahaan]', 	telp_perusahaan = '$_POST[telp_perusahaan]', fax_perusahaan = '$_POST[fax_perusahaan]',
                            jenis_usaha = '$_POST[jenis_usaha]', bidang_pekerjaan = '$_POST[bidang_pekerjaan]', jabatan = '$_POST[jabatan]', status = '$_POST[status]', masa_kerja = '$_POST[masa_kerja]', nm_atasan = '$_POST[nm_atasan]', telp_atasan = '$_POST[telp_atasan]', nm_kep_personalia = '$_POST[nm_kep_personalia]',
                            telp_personalia = '$_POST[telp_personalia]', nm_company_before = '$_POST[nm_company_before]', masa_kerja_before = '$_POST[masa_kerja_before]' where id_pekerjaan = '$_POST[id_pekerjaan]");

            //kondisi apakah save berhasil
            if($updatepekerjaan){
                echo "<script type='text/javascript'> setTimeout(function () { swal({ title: 'Update data pekerjaan berhasil', text:  'Mohon tunggu', type: 'success', timer: 3000, showConfirmButton: false }); },10); window.setTimeout(function(){
                    window.location.replace('index.php?menu=apply_credit&data=3'); } ,2000); </script>";
            } else {
                echo "<script type='text/javascript'> swal({ title: 'Error!', text: 'Update data pekerjaan gagal', type: 'error', confirmButtonText: 'Ok' }); </script>";
            }
        }
    } else {
    ...
    if(isset($_POST['savepenghasilan'])){
        if($_POST['id_penghasilan'] == ""){
            ...
            $updatepenghasilan = mysql_query("update tb_penghasilan set pendapatan_pokok = '$_POST[pendapatan_pokok]', tunjangan = '$_POST[tunjangan]', penghasilan = '$_POST[penghasilan]', total_penghasilan = '$_POST[total_penghasilan]',
                                biaya_rutin = '$_POST[biaya_rutin]', angsuran_lain = '$_POST[angsuran_lain]', biaya_lain = '$_POST[biaya_lain]', total_pengeluaran = '$_POST[total_pengeluaran]' where id_penghasilan = '$_POST[id_penghasilan]')");
            
            //kondisi apakah save berhasil
            if($updatepenghasilan){
            echo "<script type='text/javascript'> setTimeout(function () { swal({ title: 'Simpan data penghasilan berhasil', text:  'Mohon tunggu', type: 'success', timer: 3000, showConfirmButton: false }); },10); window.setTimeout(function(){
                    window.location.replace('index.php?menu=apply_credit&data=4'); } ,2000); </script>";
            } else {
            echo "<script type='text/javascript'> swal({ title: 'Error!', text: 'Simpan data penghasilan gagal', type: 'error', confirmButtonText: 'Ok' }); </script>";
    ...
    if(isset($_POST['apply'])){
        ...
        if (($identitas['mime'] == "image/jpeg" || "image/png") || ($kk['mime'] == "image/jpeg" || "image/png") || ($surat_nikah['mime'] == "image/jpeg" || "image/png") || ($sk['mime'] == "image/jpeg" || "image/png") || ($npwp['mime'] == "image/jpeg" || "image/png") || ($rekening['mime'] == "image/jpeg" || "image/png")){
            ...
            if($move1 && $move2 && $move3 && $move4 && $move6){
                //save kredit
                $savekredit = mysql_query("insert into tb_kredit values ('', $idnasabah, 2, '$_POST[permohonan_kredit]', '$_POST[jangka_waktu]', '2.95%')");
                ...
                $savestatus = mysql_query("insert into tb_status_validasi values ('', $getId, 2, '0', '-', now())");
